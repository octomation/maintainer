...
updatedAt: 2023-01-17T06:23:16Z
lastEditedAt:
closedAt:
---
...
The refined prototype from the specifications, still absent from the CLI:
...
**Boundaries:** fetch updates refs and performs no merge, reset or push; backing up issues and projects is [#31](issue-31.md). Configuring `upstream` and `fork-*` remains an original requirement of this umbrella task, but is not automatically part of the first fetch PoC.

Requirement sources: [fetch](<../notes/Specs/GitHub fetcher, PoC implementation plan.md>), [status](<../notes/Specs/Repository status, PoC implementation plan.md>), [workspace](<../notes/Specs/Workspace, implementation plan.md>), [project relations](<../notes/Issues/project graph.md>). Despite the implementation notes in those documents, the current root CLI does not contain these commands.
