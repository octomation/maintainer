...
updatedAt: 2026-09-25T05:12:48Z
lastEditedAt: 2026-09-25T05:12:48Z
...
`--short` drops the table. The date goes to stdout and the explanation `Suggestion is …, actual → target` to stderr, so it does not end up in the variable. `--delta` returns a relative value instead of an absolute timestamp. The original example with an elaborate Git alias is replaced by direct shell substitution: `git at` and `git contribute` are external wrappers.
